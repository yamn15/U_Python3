...
### 本来ならinner func「a + b」に「1, 2」が入るところだが、実行はされない→return inner2に実行を意味する「()」がついていないから
#### inner funcはfに渡されて、未実行のまま待機しているイメージ
##### fが実行された際に、innerがようやく実行される→rに3(inner関数の実行結果)が渡されて出力される
def outer2(a, b):
...
# 利用するケース
## 例えば1＋2を後から実行したいとき。(ここでは***を出力するプリントを間に挟んでいる)
